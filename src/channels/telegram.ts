...
export class TelegramChannel extends BaseChannel {
  ...
  private transcriber: GroqTranscriptionProvider | null = null;
  ...
  async start(): Promise<void> {
    ...
    this.bot.command('start', async (ctx) => {
      ...
      if (user) {
        await ctx.reply(
          `Hi ${user.first_name}! I'm ingenium.\n\nSend me a message and I'll respond!`
        );
      }
    });
  ...
  private async handleTelegramMessage(ctx: Context): Promise<void> {
    const message = ctx.message as Message.TextMessage | Message.PhotoMessage | Message.VoiceMessage | Message.DocumentMessage | undefined;
    const user = ctx.from;
    ...
    const isMediaFile = (obj: unknown): obj is MediaFileBase =>
      typeof obj === 'object' &&
      obj !== null &&
      'file_id' in obj &&
      typeof (obj as Record<string, unknown>)['file_id'] === 'string';

    let mediaFile: MediaFileBase | null = null;
    ...
    const messageOptions: Parameters<typeof this.handleMessage>[0] = {
      senderId,
