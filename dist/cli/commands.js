 ...
 */
async function createWorkspaceTemplates(workspace) {
    const templates = {
...
async function gatewayCommand(options) {
    ...
    const cronJobCallback = async (job) => {
    ...
    const heartbeatCallback = async (prompt) => {
 ...
 */
async function channelsStatusCommand() {
    const config = await loadConfig();
 ...
 * Cron remove command.
 */
async function cronRemoveCommand(jobId) {
    const storePath = join(getDataDir(), 'cron', 'jobs.json');
    const service = new CronService({ storePath });
 ...
 * Cron enable command.
 */
async function cronEnableCommand(jobId, options) {
    const storePath = join(getDataDir(), 'cron', 'jobs.json');
    const service = new CronService({ storePath });
 ...
 * Cron run command.
 */
async function cronRunCommand(jobId, options) {
    const storePath = join(getDataDir(), 'cron', 'jobs.json');
    const service = new CronService({ storePath });
...
export function buildProgram() {
    ...
    program
        ...
        .action((options) => void gatewayCommand(options));
    // Agent command
    program
        .command('agent')
        ...
        .action((options) => void agentCommand(options));
    // Status command
    program
        .command('status')
    ...
    cronCmd
        ...
        .action((options) => void cronListCommand(options));
    cronCmd
        .command('add')
