...
function htmlToMarkdown(html) {
    let text = html;
    // Convert links
    text = text.replace(/<a\s+[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi, (_, url, linkText) => `[${stripTags(linkText)}](${url})`);
    // Convert headings
...
export class WebSearchTool extends BaseTool {
    ...
    async execute(params) {
        ...
        try {
            const n = Math.min(Math.max(typeof count === 'number' ? count : this.maxResults, 1), 10);
...
export class WebFetchTool extends BaseTool {
    ...
    async execute(params) {
        ...
        const maxChars = typeof maxCharsParam === 'number' ? maxCharsParam : this.maxChars;
        try {
